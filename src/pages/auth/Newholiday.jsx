...
const CreateHolidayForm = ({ getAllHolidays }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:3000/api/superadmin/create-holiday', newHoliday, {
        headers: {
  ...
  return (
    <form className="create-holiday-form" onSubmit={handleSubmit}>
      ...
      <div>
            <label>Name:</label>
            <input type="text" name="first_name" value={newHoliday.name}   onChange={(e) => handleInputChange(e, 'name')}required />
          </div>
          <div>
            <label>Date:</label>
            <input type="text" name="email" value={newHoliday.date} onChange={(e) => handleInputChange(e, 'date')} required />
          </div>
          <div>
            <label>Description:</label>
            <input type="text" name="gender" value={newHoliday.description} onChange={(e) => handleInputChange(e, 'description')} required />
          </div>
